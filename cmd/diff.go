...
func Execute() error {
	...
	mode, err := parseCompatMode(*compatMode)
	if err != nil {
		return err
	}
...
func parseCompatMode(s string) (model.CompatMode, error) {
	switch s {
	case "backward":
		return model.ModeBackward, nil
	case "forward":
		return model.ModeForward, nil
	case "full":
		return model.ModeFull, nil
	case "none":
		return model.ModeNone, nil
	default:
		return "", fmt.Errorf("unknown compat-mode %q: must be backward, forward, full, or none", s)
	}
}
