...
func diffArrays(base, head map[string]interface{}, path string, ctx *DiffContext) []model.Change {
	...
	// A-02: type promotion (e.g. int → long) → MINOR
	if isTypePromotion(baseResolved, headResolved) {
		return []model.Change{makeChange(
			"A-02",
			path,
			fmt.Sprintf("array items type promoted from %v to %v", baseItems, headItems),
	...
	// L-01..L-07: logicalType rules on items
	if ltChanges, handled := diffLogicalTypes(baseResolved, headResolved, itemsPath, ctx); handled {
		return ltChanges
	}

	// A-01: incompatible items type change
	return []model.Change{makeChange(
		"A-01",
		path,
...
func diffMaps(base, head map[string]interface{}, path string, ctx *DiffContext) []model.Change {
	...
	// M-02: type promotion (e.g. int → long) → MINOR
	if isTypePromotion(baseResolved, headResolved) {
		return []model.Change{makeChange(
			"M-02",
			path,
			fmt.Sprintf("map values type promoted from %v to %v", baseValues, headValues),
			"Avro supports this type promotion; existing data remains readable",
			modesNone,
			ctx,
		)}
	...
	// L-01..L-07: logicalType rules on values
	if ltChanges, handled := diffLogicalTypes(baseResolved, headResolved, valuesPath, ctx); handled {
		return ltChanges
	}

	// M-01: incompatible values type change
	return []model.Change{makeChange(
		"M-01",
		path,
...
// isTypePromotion reports whether changing from → to is a valid Avro type promotion (MINOR).
// Both arguments must be primitive type strings.
// Promotions: int→{long,float,double}, long→{float,double}, float→double, string↔bytes.
func isTypePromotion(from, to interface{}) bool {
	f, ok1 := from.(string)
	t, ok2 := to.(string)
	if !ok1 || !ok2 {
		return false
